class Todo {
  ...
  render() {
    ...
    <li class="todo__item todo-item" data-js-todo-item>
        <input data-js-todo-item-checkbox class="todo-item__checkbox" id="${id}" type="checkbox" ${
          isChecked ? "checked" : ""
        }>
        <label data-js-todo-label for="${id}" class="todo-item__label">${title}</label>
        <button data-js-todo-item-delete-button class="todo-item__delete-button" type="button" aria-label="Delete"
          title="Delete">
  ...
  onSearchTaskInputSubmit = ({ target }) => {
